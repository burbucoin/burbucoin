...
uint256 hashGenesisBlock("0x7ee74d87729abaf37150b7125cca1dd9e0ea0295355cec57ba063c6c2b10cffa");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // burbucoin: starting difficulty is 1 / 2^12
...
int64 CTransaction::nMinRelayTxFee = nMinTxFee;
...
const string strMessageMagic = "burbucoin Signed Message:\n";
...
{
    // burbucoin: IsDust() detection disabled, allows any valid dust to be relayed.
...
bool CTransaction::CheckTransaction(CValidationState &state) const
{
    ...
    if (IsCoinBase())
    {
		// Temp work around, because different size in scriptSig
...
int64 static GetBlockValue(int nHeight, int64 nFees)
{
    ...
    nSubsidy >>= (nHeight / 840000); // Burbucoin: 840k blocks in ~4 years

    return nSubsidy + nFees;
}

	static int64 nTargetTimespan = 5 * 60 * 60; // Retarget every 5 hour
	static int64 nTargetSpacing = 2.5  * 60; // Burbucoin: 2.5 minutes
...
unsigned int static GetNextWorkRequired_V1(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // burbucoin: This fixes an issue where a 51% attack can change difficulty at will.
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
        ...
        else { // KGW start at block 24000
                if (pindexLast->nHeight+1 >= 24000) { DiffMode = 2; }
        }
...
int GetAuxPowStartBlock()
{
    if (fTestNet)
        ...
        return 37000; // burbucoin starts to be LTC merged from block 37,000
}

int GetOurChainID()
{
    return 0x0020; // burbucoin has got ChainID=0x0020 in LTC-merged mining network
}
...
bool CBlock::CheckBlock(CValidationState &state, int nHeight, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // burbucoin: Special short-term limits to avoid 10,000 BDB lock limit:
...
{
    // burbucoin: temporarily disable v2 block lockin until we are ready for v2 transition
    return false;
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        // Genesis block
        const char* pszTimestamp = "http://www.burbuja.info/inmobiliaria/burbuja-inmobiliaria/494162-paro-baja-mas-de-107-000-personas-diciembre-segun-servimedia.html";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        txNew.vin[0].scriptSig = CScript() << 486604799 << CBigNum(4) << vector<unsigned char>((const unsigned char*)pszTimestamp, (const unsigned char*)pszTimestamp + strlen(pszTimestamp));
        txNew.vout[0].nValue = 0 * COIN;
        txNew.vout[0].scriptPubKey = CScript() << ParseHex("0042173f794b86f51e44ea3c9deed8f7ee7b1b6621d9e8a93901658f7ba13a96b512f15b1481e3459c973b3116a045b819e057ad4bf8528b7922c7548e56cbe7651") << OP_CHECKSIG;
        CBlock block;
        block.vtx.push_back(txNew);
        block.hashPrevBlock = 0;
        block.hashMerkleRoot = block.BuildMerkleTree();
        block.nVersion = 1;
        block.nTime    = 1388726060;
        ...
        printf("%s\n", block.hashMerkleRoot.ToString().c_str());
 		assert(block.hashMerkleRoot == uint256("0x74fc0dff9ee2c823dc43508cb36525f071579b4b37c10a39dae403e033303d37"));
        block.print();
...
unsigned char pchMessageStart[4] = { 0xff, 0xc2, 0xa5, 0xad };; // burbucoin: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("burbucoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("burbucoinMiner : ProcessBlock, block not accepted");
    }
